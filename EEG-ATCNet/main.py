...
def train(dataset_conf, train_conf, results_path):
    ...
    for sub in range(n_sub): # (num_sub): for all subjects, (i-1,i): for the ith subject.
        ...
        for train in range(n_train): # How many repetitions of training for subject i.
            ...
            with tqdm(total=epochs, desc=f"Training subject {sub+1}, run {train+1}") as pbar:
                for epoch in range(epochs):
                    epoch_history = model.fit(X_train, y_train_onehot, validation_data=(X_test, y_test_onehot), 
                                        epochs=1, batch_size=batch_size, callbacks=callbacks, verbose=0)
                    # Update combined history
                    for key in combined_history.keys():
                        combined_history[key].extend(epoch_history.history[key])
                    pbar.update(1)
            ...
            if(BestSubjAcc < acc[sub, train]):
                 ...
                 bestTrainingHistory = combined_history
...
def run():
    # Get dataset path
    data_path = 'E:/deeplearning/EEG-ATCNet-main/BCI Competition IV-2a/'
